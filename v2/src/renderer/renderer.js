...
async function refreshStats() {
  state.stats = await window.api.indexGetStats();
}
...
function render() {
  ...
  window.StatsPanel({
    ...
    onRebuild: async () => {
      ...
      addLog('Przebudowuję indeks czatów...');
      render();
      try {
        const result = await window.api.indexRebuild();
        await refreshStats();
        addLog(`Indeks gotowy. Czatów: ${result.chatCount}`);
      } catch (err) {
        addLog(`Błąd indeksowania: ${err.message}`);
      }
