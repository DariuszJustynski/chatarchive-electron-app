...
const { rebuildIndex, getStats } = require('../analytics/indexManager');
...
function registerIpcHandlers(mainWindow, viewManager) {
  ...
  // Rebuild the chat index and stats by scanning all export folders.
  ipcMain.handle(IPC_CHANNELS.INDEX_REBUILD, async () => {
    const settings = await settingsStore.getSettings();
    const result = await rebuildIndex(settings.outputDir);
    return {
      chatCount: result.index.chatCount,
      generatedAt: result.index.generatedAt,
    };
  });

  // Return the current chat-stats.json content (null if not yet generated).
  ipcMain.handle(IPC_CHANNELS.INDEX_GET_STATS, async () => getStats());
}
