...
const { updateIndexWithFolder } = require('../analytics/indexManager');
...
async function runExport({ mainWindow, items, formats, outputDir, viewManager }) {
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    try {
      ...
      // --- 6. Update analytics index (fire-and-forget, non-blocking) -------
      updateIndexWithFolder(dirPath).catch((err) =>
        console.error('[indexManager] incremental update failed:', err),
      );
